...
public class DebugLogs implements Process {
    ...
    private Response download() {
        ...
        response.addHeader("Content-Disposition", "attachment; filename=xctv-debug-log.txt");
    ...
    private String html() {
        ...
        return "<!doctype html>"
                ...
                + "<header><h1>调试日志</h1><a href=\"/debug/logs" + auth + "\">刷新</a><a id=\"download\" href=\"/debug/logs.txt" + auth + "\" download=\"xctv-debug-log.txt\">下载</a><a href=\"/debug/clear" + auth + "\">清空</a><a href=\"" + (enabled ? "/debug/disable" : "/debug/enable") + auth + "\">" + (enabled ? "关闭" : "开启") + "</a><span id=\"meta\" class=\"meta\" data-version=\"" + DebugLogStore.version() + "\">" + (enabled ? "开启" : "关闭") + " · " + DebugLogStore.size() + " 行 · " + DebugLogStore.bytes() / 1024 + " KB</span></header>"
                + "<main><details class=\"info\"><summary>地址和说明</summary><p class=\"hint\">本页显示 App 当前进程内调试日志。开启后记录安卓系统版本、设备型号、WebView 版本、XCTV、SDK、HTTP 服务、爬虫请求和播放链路；最多保留最近 2000 行，关闭会自动清空。</p>"
                + "<div class=\"addr\"><a href=\"" + escape(localUrl) + "\">本机地址：" + escape(localUrl) + "</a><a href=\"" + escape(lanUrl) + "\">局域网地址：" + escape(lanUrl) + "</a></div></details>"
                + "<section class=\"tools\"><div class=\"chips\"><button class=\"chip on\" data-mode=\"all\">全部</button><button class=\"chip\" data-mode=\"proxy\">代理</button><button class=\"chip\" data-mode=\"player\">播放</button><button class=\"chip\" data-mode=\"webhome\">XCTV</button><button class=\"chip\" data-mode=\"console\">Console</button><button class=\"chip\" data-mode=\"webview\">WebView</button><button class=\"chip\" data-mode=\"api\">站源</button><button class=\"chip\" data-mode=\"pan\">网盘</button><button class=\"chip\" data-mode=\"server\">服务</button><button class=\"chip\" data-mode=\"sync\">同步</button><button class=\"chip\" data-mode=\"startup\">启动</button><button class=\"chip\" data-mode=\"error\">错误</button></div>"
    ...
    private String scriptEnhanced(String token) {
        return "const rawEl=document.getElementById('raw'),logs=document.getElementById('logs'),meta=document.getElementById('meta'),summary=document.getElementById('summary'),filter=document.getElementById('filter'),simple=document.getElementById('simple'),pause=document.getElementById('pause'),download=document.getElementById('download');"
                ...
                + "if(r.tag&&r.tag.startsWith('webhome')){e.kind='webhome';e.state=r.msg.includes('-> 200')||r.msg.includes('invoke')?'ok':'raw';e.badge='XCTV';e.title=r.tag==='webhome-net'?(r.msg.includes('->')?'XCTV 网络响应':'XCTV 网络请求'):'XCTV 开放能力调用';e.detail=r.msg;return e}"
